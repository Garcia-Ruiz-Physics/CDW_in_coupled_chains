%% E_electronic for N skewed-coupled chains: NUMERICAL APPROACH
% This programme computes the total energy for 
% two skewed-coupled chains with a CDW order.

%% clear the workspace
clear all
close all
clc
%% Defining constants
t=1;
a=1;
%% Defining problem parameters
stepD=5*10^-4;
MaxD=0.1;
stepT=10^-3;
MaxT=0.025;
D=[];
D0=0.1:0.1:9;
for OoM=-4:2:-2
    D=[D D0*10^OoM];
end
D=[0 D 0.091:0.001:0.1];
lD=length(D);
% Inter-chain coupling
tperp=0:stepT:MaxT;
lT=length(tperp);
doping=0:0.001:0.01;
lDO=length(doping);
% momentum grid
stepK=500*10^-6;
k_list=(-1:stepK:0)*2*pi/(2*a)/2;
Kdiff=k_list(2)-k_list(1);
lK=length(k_list);
for N=2:6 % Number of chains
    Ee_nem=zeros(lT,lD,lDO);
    Ee_zzg=zeros(lT,lD,lDO);
    for T=1:lT % For every inter-chain coupling
        for d=1:lD % For every Peierls CDW order
            Enem=zeros(lK,2*N);
            Ezzg=zeros(lK,2*N);
            for KKK=1:lK
                % For every momentum point
                %% 1D atomic chain
                f=(t+D(d))+(t-D(d))*exp(2i*k_list(KKK)*a);
                H_1D=[0  f;
                      f' 0];
                Vs=[1 exp(2i*k_list(KKK)*a);
                    1 1                   ]*tperp(T);
                %% Decoupled Hamiltonian
                H_dec=[];
                for n=1:N
                    H_dec=blkdiag(H_dec,H_1D);
                end
                %% Nematic phase
                T_nem=[];
                for NNN=1:N-1
                    T_nem=blkdiag(T_nem,Vs);
                end
                T_nem=[zeros(2*(N-1),2) T_nem;
                    zeros(2,2*N)];
                T_nem=T_nem+T_nem';
                H_nem=H_dec+T_nem;
                %% Zig-Zag phase
                T_zzg=[];
                for NNN=1:N-1
                    if NNN/2==floor(NNN/2)
                        T_zzg=blkdiag(T_zzg,Vs');
                    else
                        T_zzg=blkdiag(T_zzg,Vs);
                    end
                end
                T_zzg=[zeros(2*(N-1),2) T_zzg;
                    zeros(2,2*N)];
                T_zzg=T_zzg+T_zzg';
                H_zzg=H_dec+T_zzg;
                %% Diagonalizing the Hamiltonian
                Enem(KKK,:)=eig(H_nem);
                Ezzg(KKK,:)=eig(H_zzg);
            end
            %% Computing the EF for zzg and nem
            for DO=1:lDO
                ExtraPoints=floor(lK*doping(DO));
                % Nematic configuration
                Values_nem=reshape(Enem,1,lK*2*N);
                Values_nem_sorted= sort(Values_nem);
                EF_nem=(Values_nem_sorted(lK*N+ExtraPoints)+...
                        Values_nem_sorted(lK*N+ExtraPoints+1))/2;
                % Zig-Zag configuration
                Values_zzg=reshape(Ezzg,1,lK*2*N);
                Values_zzg_sorted= sort(Values_zzg);
                EF_zzg=(Values_zzg_sorted(lK*N+ExtraPoints)+...
                        Values_zzg_sorted(lK*N+ExtraPoints+1))/2;
                %% Total electronic energy up to some kc value (see k)
                Total_Enem_list=sum( Values_nem.*(Values_nem<EF_nem) );
                Ee_nem(T,d,DO)=2*...% Two "valleys"
                    (Total_Enem_list)... % Sum over all bands
                    /(2*lK); % Totsl number of unit cells
                Total_Ezzg_list=sum( Values_zzg.*(Values_zzg<EF_zzg) );
                Ee_zzg(T,d,DO)=2*...% Two "valleys"
                    (Total_Ezzg_list)... % Sum over all bands
                    /(2*lK); % Total number of unit cells
            end
        end
        disp([num2str(T/lT*100),' % done'])
    end
    save(['Electronic_contribution_Half_Filled_skewed_N_',num2str(N)], ...
        'N','Ee_zzg','Ee_nem','tperp','D','stepK','doping')
end
