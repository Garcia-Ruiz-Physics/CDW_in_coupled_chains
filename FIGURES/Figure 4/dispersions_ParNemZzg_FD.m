%% Total energy of N-skewed-coupled atomic chains: numerical
% This programmes computes the total energy of an N-coupled dimerised
% atomic chain.
%% INPUTS:
% N: number of chains
...
for A=1:laX
    ...
    if A==1
        hold on
        for B=1:2*N
            plot(k_list/(pi),Enem(1,:,B),'k-','LineWidth',0.5)
        end
    ...
    elseif A==laX
        hold on
        for B=1:2*N
            % plot(k_list/pi,Enem(A,:,B),'color',green,'LineWidth',2)
